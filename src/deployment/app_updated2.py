...
def engineer_features(data_dict):
    """Compute features from TX_DATETIME (same as training)"""
    import pandas as pd
    
    # Extract time features from TX_DATETIME
    if 'TX_DATETIME' in data_dict:
        dt = pd.to_datetime(data_dict['TX_DATETIME'])
        data_dict['hour'] = dt.hour
        data_dict['day'] = dt.day
        data_dict['month'] = dt.month
        data_dict['weekday'] = dt.weekday()
        data_dict['is_weekend'] = 1 if dt.weekday() >= 5 else 0
        data_dict['is_night'] = 1 if (dt.hour >= 22 or dt.hour < 6) else 0
    ...
    data_dict['TX_TIME_SECONDS'] = dt.hour * 3600 + dt.minute * 60 + dt.second
    # = 14 * 3600 + 30 * 60 + 0 = 52,200
    
    # And TX_TIME_DAYS (if needed):
    dataset_start = pd.to_datetime('2018-04-01')
    data_dict['TX_TIME_DAYS'] = (dt - dataset_start).days
